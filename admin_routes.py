...
@admin_bp.route('/admin/remove-service', methods=['GET', 'POST'])
@login_required
def remove_service():
    if request.method == 'POST':
        service_id = request.form.get('service_id')
        try:
            result = mongo.db.card.delete_one({'_id': ObjectId(service_id)})
            if result.deleted_count > 0:
                print(f"Service with ID {service_id} removed successfully")
            else:
                print(f"No service found with ID {service_id}")
        except Exception as e:
            print(f"Error removing service: {e}")
        return redirect('/admin/remove-service')
    else:
        # GET request - show all services
        cards = list(mongo.db.card.find())
        return render_template('remove_service.html', cards=cards)
